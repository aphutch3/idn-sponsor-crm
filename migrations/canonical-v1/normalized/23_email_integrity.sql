-- Canonical-v1 corrective migration against merged Phase 8 commit 93e9c0e.
-- Derived from tested P8R2, preserving the production send_status enum.
BEGIN;
SET LOCAL search_path = public, pg_catalog;
SET LOCAL lock_timeout='5s';
LOCK TABLE public.campaign_send,public.campaign_send_event IN ACCESS EXCLUSIVE MODE;
DO $$ BEGIN
 IF EXISTS(SELECT 1 FROM public.campaign_send_event) THEN
   RAISE EXCEPTION 'Email history changed: review reconciliation before installing this correction';
 END IF;
END $$;
DROP TRIGGER campaign_send_event_rollup ON public.campaign_send_event;
...
BEGIN
  ...
  next_status := CASE NEW.event_kind
    ...
    WHEN 'bounced' THEN 'bounced' WHEN 'failed' THEN 'failed' ELSE s.status::text END;
  IF NEW.event_kind='delivered' AND s.delivered_at IS NULL THEN delivery_increment:=1; END IF;
  UPDATE public.campaign_send SET
    status = CASE WHEN public.p8r_status_rank(next_status)>public.p8r_status_rank(status::text)
                  THEN next_status::public.send_status ELSE status END,
